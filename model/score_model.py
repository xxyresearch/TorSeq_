...
# Part of code from https://github.com/gcorso/torsional-diffusion
# Part of code from https://github.com/hojonathanho/diffusion/blob/master/diffusion_tf/nn.py
# Part of Code are changed for our method
...
class TensorProductScoreModel(torch.nn.Module):
    def __init__(self, in_node_features=74, in_edge_features=4, sigma_embed_dim=32, sigma_min=0.01 * np.pi,
                 ...
                 scale_by_sigma=True, use_second_order_repr=True, batch_norm=True, residual=True, rnn = True, rnn_layers =2
                 ):
        ...
        if rnn:
            self.rnn_layer = nn.LSTM(input_size = ns, 
                    hidden_size = ns, 
        ...
        self.final_linear = nn.Sequential(
            nn.Linear(final_in_dim, ns, bias=False),
            nn.Tanh(),
    ...
    def forward(self, data):
        ...
        if self.rnn:
            torsion_attr = out [data.torsion_rnn_index]*data.seq_mask.float().unsqueeze(-1)
            packed_torsion_attr  = pack_padded_sequence(torsion_attr, data.rnn_length.detach().cpu(), batch_first=True, enforce_sorted=False) 
            packed_torsion_attr, (hn, cn) = self.rnn_layer(packed_torsion_attr)
            torsion_attr, output_lengths = pad_packed_sequence(packed_torsion_attr, batch_first=True)       
            max_length = torch.max(output_lengths)
            mask = data.seq_mask[:,:max_length]
            torsion_attr = torsion_attr[mask] + torch.cat((out, out),1)
            out = self.final_linear(torsion_attr)
        else:
